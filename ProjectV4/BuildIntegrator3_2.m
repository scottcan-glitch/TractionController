...
% syms
Xk = MX.sym('Xk',5,1);  % initial state for solve
...
for k=0:Nh-1
    ...
    % nlp.g = [nlp.g, { Uk(4) - (Xk_end(3) - Xk(3))/Ts }];
    % args.lbg = [args.lbg; 0];
    % args.ubg = [args.ubg; 0];
    ...
    % Add longitudinal tire slip constraint (<20%)
    wc_2_vx = Vx - (tw/2)*r;
    wc_3_vx = Vx + (tw/2)*r;
...
x0_val = [10;0;0;10/sedan1.geometry.r_wheel;10/sedan1.geometry.r_wheel];
% Reference track
xref_val = [40;0;0;40.5/sedan1.geometry.r_wheel;40.5/sedan1.geometry.r_wheel];
[xref_val, uref_val] = get_u_ref(xref_val,[26;26;0;0])
% Compile into parameter vector p0 = [x0_val; xref_val; uref_val]
p0_val = [x0_val; xref_val; uref_val];
% Warm Start MPC (initial solver guess)
w0_val = repmat([uref_val; x0_val],Nh,1);
...
for k = 1:Nsim
    ...
    p0_val = [x_kp1; xref_val; uref_val]
